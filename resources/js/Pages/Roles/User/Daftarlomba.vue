...
async function submitForm() {
    try {
        // Submit form data to `/reg_lomba` endpoint
        await router.post('/daftarlomba', {
            user_id: userData.id,
            team_id: team.id,
            lomba_id: team?.lomba?.id,
            team_member_id: teamMembers.value.map(member => member.id),
            submission_id: submissions.id // Assuming `submissions` object is passed correctly
        });
        Swal.fire({
            title: 'Berhasil!',
            ...
            confirmButtonText: 'OK',
            onClose: () => {
                // Redirect to '/daftarlomba' after successful submission
                router.push('/daftarlomba');
            }
        });
    } catch (error) {
